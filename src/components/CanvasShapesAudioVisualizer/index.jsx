...
export const AudioVisualizer = ({ numberOfShapes = 15, shadows = false }) => {
    const { isAudioPlaying, audioContext } = useSharedAudio();
    const { analyser } = useAudioAnalysis();
    const canvasRef = useRef(null);
    const bounds = useMemo(() => ({ minX: 0, minY: 0, maxX: window.innerWidth, maxY: window.innerHeight }), []);
    ...
    return (<canvas ref={canvasRef} className="fixed -z-10 top-0 left-0 w-full min-h-screen bg-[#303a4d]"></canvas>);
};
function getBassFrequency(audioData) {
    const bassFrequency = audioData.slice(0, 10).reduce((acc, curr) => acc + curr, 0) / 10;
    return bassFrequency;
}
function drawFrequencyLineGraph(dataArray, ctx, width, height) {
