...
export const AudioVisualizerWithMatter = () => {
    ...
    const bodiesRef = useRef([]); // Ref to store the bodies
    ...
    useEffect(() => {
        ...
        const bodies = Matter.Composites.stack(20, 100, 15, 3, 20, 40, function (x, y) {
            ...
            switch (bodyType) {
                case "rectangle":
                    ...
                    return Matter.Bodies.rectangle(x, y, Matter.Common.random(20, 40), Matter.Common.random(20, 40), bodyOptions);
                case "polygon":
                    // Create a polygon with random sides and radius, minimum 3 sides
                    return Matter.Bodies.polygon(x, y, Math.floor(Matter.Common.random(3, 6)), Matter.Common.random(10, 20), bodyOptions);
                case "circle":
                default:
    ...
    useEffect(() => {
        if (bodiesRef.current.length > 0) {
            bodiesRef.current.forEach((body) => {
                ...
                const forceMagnitude = forceMagnitudeMin + (forceMagnitudeMax - forceMagnitudeMin) * normalizedBassLevel;
                // Apply force in a more varied direction based on the calculated force magnitude
                Matter.Body.applyForce(body, body.position, {
                    x: (forceMagnitude + Matter.Common.random() * forceMagnitude) *
    ...
    return (_jsx("div", { ref: visualizerRef, style: {
            position: "absolute",
            top: 0,
            left: 0,
            width: "100vw",
...
//# sourceMappingURL=MatterAudioVisualizer.js.map